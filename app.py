...
# ==========================================
st.set_page_config(page_title="手搖飲全知戰情室 (黑曜石版)", page_icon="🧋", layout="wide")
...
st.markdown("""
    ...
    /* 強制 Streamlit 元生標籤、文字在深色背景下清晰呈現 */
...
st.title("🧋 台灣手搖飲商業分析戰情室 (黑曜石究極版)")
st.markdown("<p style='font-size:16px; color:#94A3B8;'>融合 3D 立體版圖、AI 決策大腦、定價沙盤推演與消費者行為學的 <b>神級商業決策系統</b>。</p>", unsafe_allow_html=True)
...
with tab2:
    ...
    if not quad_df.empty:
        ...
        fig_3d.update_layout(scene=dict(
            xaxis=dict(title='大杯均價 (X)', backgroundcolor="#0F172A", gridcolor="#334155", showbackground=True, titlefont=dict(color="#94A3B8"), tickfont=dict(color="#94A3B8")),
            yaxis=dict(title='品項豐富度 (Y)', backgroundcolor="#1E293B", gridcolor="#334155", showbackground=True, titlefont=dict(color="#94A3B8"), tickfont=dict(color="#94A3B8")),
            zaxis=dict(title='加料佔比% (Z)', backgroundcolor="#0F172A", gridcolor="#334155", showbackground=True, titlefont=dict(color="#94A3B8"), tickfont=dict(color="#94A3B8")),
    ...
    if not gap_analysis.empty:
        ...
        with rd_c1:
            ...
            fig_gap.update_layout(
                scene=dict(
                    xaxis=dict(title='市場競爭度 (商品數)', backgroundcolor="#0F172A", gridcolor="#334155", showbackground=True, titlefont=dict(color="#94A3B8"), tickfont=dict(color="#94A3B8")),
                    yaxis=dict(title='定價天花板 (均價)', backgroundcolor="#1E293B", gridcolor="#334155", showbackground=True, titlefont=dict(color="#94A3B8"), tickfont=dict(color="#94A3B8")),
                    zaxis=dict(title='藍海潛力指數', backgroundcolor="#0F172A", gridcolor="#334155", showbackground=True, titlefont=dict(color="#94A3B8"), tickfont=dict(color="#94A3B8")),
        ...
        with rd_c2:
            ...
            st.markdown(f"""
            <div style="background: linear-gradient(145deg, #020617, #1E293B); color: #F8FAFC; padding: 25px; border-radius: 16px; box-shadow: 0 10px 30px rgba(0,0,0,0.5); border-top: 4px solid #4F46E5;">
